...
class Dataset(object):
    ...
    def load_data(self, test_id, is_preprocess=True, is_sub_center=False):
    ...
    def load_data_with_vae_feature(self, test_id, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize, is_preprocess=True):
        assert(test_id >= 1 and test_id <= self.SUBJECT_NUM)
        x_train_skeleton = []
        x_train_global = []
        x_train_hand = []
        ...
        for i in xrange(1, self.SUBJECT_NUM + 1):
            ...
            for info in self.info[i]:
                ...
                feature_hand, feature_global = \
                    mf.extract_all_feature_for_frame_vae(pose, test_id, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize, vae)
                if i == test_id:
        ...
        for i in xrange(1, self.SUBJECT_NUM + 1):
            ...
            for info in self.info[i]:
                ...
                label = ((info[0] - 1) * self.FINGER_NUM + info[1] - 1
                         if self.is_full else info[0] - 1)
                feature_hand, feature_global = \
                    mf.extract_all_feature_for_frame_wo_abduction(pose, test_id, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize)
                if i == test_id:
                    x_test_skeleton.append(pose)
                    x_test_global.append(feature_global)
                    x_test_hand.append(feature_hand)
