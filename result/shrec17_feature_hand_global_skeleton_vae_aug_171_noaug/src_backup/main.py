...
def parse_arguments():
    ...
    parser.add_argument('-online', '--online_augmentation', dest='online', action='store_true')
...
def get_three_branches_model_attention(input_dim, class_num, lr, is_bid):
    rnn_cells = [(LSTM, 100, 0.5), (LSTM, 100, 0.5)]
    rnn_cells2 = [(LSTM, 100, 0.3), (LSTM, 100, 0.3)]
    rnn_cells3 = [(LSTM, 100, 0.3), (LSTM, 100, 0.3)]
    fc_dims = [(128, 0.5)]
    return rnn_model.get_model_three_branches_new_attention(input_dim[0], rnn_cells, input_dim[1], rnn_cells2, 66, rnn_cells3, fc_dims, class_num,
             is_bid=False, optimizer=Adam(lr=lr, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0))
...
def main(root_dir, test_id, is_full, lr, batch_size, num_epoch, save_postfix, mode, data,
         M, max_dist_factor, offset1, offset2, MAX_LEN, is_bid, save_best_only, dataset, online):
    # mkdir
...
#    os.system(cmd)
    ...
    if data == 0:
        ...
        #                                                        is_preprocess=True)
        _, (x_test_all, y_test) = data.load_data_with_vae_feature(test_id, do_pca, pca_dim, M,
                                                                   max_dist_factor,
                                                                   offset1, offset2, is_normalize,
                                                                   is_preprocess=True)
        print 'elapsed time: {} s.'.format(time.time() - start_time)
        start_time = time.time()
        model, graph = get_three_branches_model_new(data.input_dim, data.class_num, lr, is_bid)
    elif data == 7:
        if dataset == 'SHREC17':
            data = gesture_dataset_shrec17.Dataset(root_dir, is_full, is_aug=False)
            (x_train_all, y_train), (x_test_all, y_test) = data.load_data_with_vae_feature(do_pca, pca_dim, M, max_dist_factor,
                                                                           offset1, offset2, is_normalize, is_preprocess=True,
                                                                                           )# use_denoised_pose = True)
        else:
            data = gesture_dataset.Dataset(root_dir, is_full)
            (x_train_all, y_train), (x_test_all, y_test) = data.load_data_with_vae_feature(test_id, do_pca, pca_dim, M, max_dist_factor,
                                                                           offset1, offset2, is_normalize, is_preprocess=True, offline_aug=False)
        ...
        if dataset == 'SHREC17':
            data = gesture_dataset_shrec17.Dataset(root_dir, is_full, is_aug=True)
            (x_train_all, y_train), (x_test_all, y_test) = data.load_data_with_vae_feature(do_pca, pca_dim, M,
                                                                                           max_dist_factor,
                                                                                           offset1, offset2,
                                                                                           is_normalize,
                                                                                           is_preprocess=True,
            ...
            data = gesture_dataset_aug.Dataset(root_dir, is_full, is_aug=True)
            (x_train_all, y_train), (x_test_all, y_test) = data.load_data_with_vae_feature(test_id, do_pca, pca_dim, M,
                                                                                           max_dist_factor,
                                                                                           offset1, offset2,
                                                                                           is_normalize,
                                                                                           is_preprocess=True)
        ...
        start_time = time.time()
        # model, graph = get_three_branches_model_new(data.input_dim, data.class_num, lr, is_bid)
        model = get_three_branches_model_attention(data.input_dim, data.class_num, lr, is_bid)
        graph = None
    elif data == 10: # no amp
        if dataset == 'SHREC17':
            data = gesture_dataset_shrec17.Dataset(root_dir, is_full, is_aug=True)
            ...
            data = gesture_dataset.Dataset(root_dir, is_full)
            (x_train_all, y_train), (x_test_all, y_test) = data.load_data_with_vae_feature_noamp(test_id, do_pca, pca_dim, M,
                                                                                           max_dist_factor,
                                                                                           offset1, offset2,
                                                                                           is_normalize,
                                                                                           is_preprocess=True,
        ...
        if dataset == 'SHREC17':
            ...
            data = gesture_dataset.Dataset(root_dir, is_full)
            (x_train_all, y_train), (x_test_all, y_test) = data.load_data_with_vae_feature_oriamp(test_id, do_pca, pca_dim, M,
                                                                                           max_dist_factor,
                                                                                           offset1, offset2,
                                                                                           is_normalize,
                                                                                           is_preprocess=True,
    ...
    data_gen = None#GestureDataGenerator(test_id, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize)
    print 'elapsed time: {} s.'.format(time.time() - start_time)
    ...
    if mode == 0:
        if not online:
            ...
            rnn_model.train_model_online_feature(model, graph, x_train_all, y_train, x_test_all, y_test,
                                 data.class_num, batch_size, num_epoch, save_postfix, MAX_LEN, save_best_only, test_id, data_gen)
