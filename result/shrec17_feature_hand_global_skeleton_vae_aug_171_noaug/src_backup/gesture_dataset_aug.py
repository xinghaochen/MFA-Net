...
class Dataset(object):
    ...
    def __init__(self, root_dir, is_full, is_aug = False, use_image=False):
    ...
    def load_data(self, test_id, is_preprocess=True, is_sub_center=False, offline_aug=True):
    ...
    def load_data_with_vae_feature_bak(self, test_id, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize,
    ...
    def load_data_with_vae_feature(self, test_id, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize,
        ...
        for i in xrange(1, self.SUBJECT_NUM + 1):
            ...
            for info in self.info[i]:
                if cnt%10 == 0:
                    print '\t {}/{} loaded.'.format(cnt, self.aug_num*len(self.info[i]))
                    sys.stdout.flush()
                    #break
                pose = self._get_pose(info)
                label = ((info[0] - 1) * self.FINGER_NUM + info[1] - 1
                ...
                for aug_id in xrange(self.aug_num):
                    ...
                    # print 'pose_aug shape', pose_aug.shape
                    # print 'pose_aug', aug_id, pose_aug
                    feature_hand, feature_global = \
                        mf.extract_all_feature_for_frame(pose_aug, 0, do_pca, pca_dim, M, max_dist_factor, offset1, offset2, is_normalize)
                    if i == 1:
                        x_test_skeleton.append(pose_aug)
                        x_test_global.append(feature_global)
                        x_test_hand.append(feature_hand)
