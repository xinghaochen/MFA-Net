...
from vis_util import plot_hand_pose
...
class PoseVAE(object):
    ...
    def test(self, x_test):
        ...
        for i in xrange(10):#x_test_encoded.shape[0]):
                ...
                np.savetxt('visualization/vae/vae_pose_{}.txt'.format(i), pose)
                np.savetxt('visualization/vae/vae_pose_{}_gt.txt'.format(i), pose_gt)
                #show_two_skeleton(pose, pose_gt, is_show_connect=1, is_show_id=0)
                #plt.savefig('visualization/vae/vae_pose_{}.png'.format(i), transparent=True)
                #plt.savefig('visualization/vae/vae_pose_{}.pdf'.format(i), transparent=True)
